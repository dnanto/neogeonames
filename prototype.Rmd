...
admin.codify.1 <- function(val, cc2 = NA, n = 1)
{
  ac1 <- NA
  
  df <- filter(df.admin.1, is.na(cc2) | iso == cc2)
...
process_regx <- function(val, regx)
{
  ...
  if (!any(is.na(tokens)))
  {
    ...
    {
      if (name == "cn") result[[name]] <- countrify(tokens[[name]])
      else if (name == "ac1")
      {
        result[[name]] <- admin.codify.1(tokens[[name]], cc2 = result$cn)
      }
    }
  }
...
process_regx("usa: GA", list(pattern = "(.+):\\s*(.+)", names = c("cn", "ac1")))
```

```{r}
x <- filter(df.src, startsWith(country, "USA"))

pull(x, country) %>% 
  lapply(process_regx, regx = list(pattern = "(.+):\\s*(.+)", names = c("cn", "ac1"))) %>% 
  bind_rows() %>% 
  bind_cols(x, .) %>%
