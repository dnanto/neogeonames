...
```{r}
df.src <- read_tsv("data/sample/src.zip", col_types = cols(.default = "c")) %>% distinct(country)
```
...
country.normalize <- function(query, n = 1, ...)
{
  ...
  if (!is.na(query))
  {
    ...
    if (is.na(i) )
    {
      tokens <- str_split(query, " ")[[1]]
      for (ele in tokens)
      {
        result <- country.normalize(ele)
        if (!is.na(result)) 
        {
          i <- result
          break
...
country.normalize.vec <- Vectorize(country.normalize, c("query"), USE.NAMES = F)
x <- mutate(df.src, clean = df.country$country[country.normalize.vec(country.sanitize.vec(country))])
x
```
